...
/* The time axis is a focusable role="slider" and it answered no key at all.
 *
 * The rail next to it handles six. This one you could Tab to, hear announced as
 * a slider, and press every arrow on with nothing happening - which is worse
 * than not being focusable, because it advertises an affordance it does not
 * have. The only way to move the window was a pointer.
 *
 * Same asinh arithmetic as the wheel, so the keyboard and the wheel agree about
 * what a zoom is, and both stay anchored the way a drag does. */
function zoomWindow(anchorT, f) {
  const k = Math.max((S.win.t1 - S.win.t0) / 46, 1e-9);
  const up = Math.asinh(Math.max(0, anchorT) / k);
  const u0 = Math.asinh(S.win.t0 / k), u1 = Math.asinh(S.win.t1 / k);
  setWindow(Math.max(0, k * Math.sinh(up + (u0 - up) * f)),
            Math.min(T_MAX, k * Math.sinh(up + (u1 - up) * f)));
}
...
ccv.addEventListener('keydown', e => {
  ...
  const mid = () => {
    const sc = SCALE || chronScale();
    return Math.max(0, sc.t(CW / 2));
  };
  // Right is toward the present, which is toward smaller ybp.
  if (e.key === 'ArrowRight') panWindow(big ? -0.4 : -0.12);
  else if (e.key === 'ArrowLeft') panWindow(big ? 0.4 : 0.12);
  else if (e.key === 'ArrowUp' || e.key === '+' || e.key === '=') zoomWindow(mid(), big ? 0.7 : 0.862);
  else if (e.key === 'ArrowDown' || e.key === '-') zoomWindow(mid(), big ? 1.43 : 1.16);
  else if (e.key === 'PageUp') zoomWindow(mid(), 0.5);
  else if (e.key === 'PageDown') zoomWindow(mid(), 2);
  else if (e.key === 'Home') setWindow(0, T_MAX);
  else if (e.key === 'End') setWindow(0, 130);
  else return;
  e.preventDefault();
});
...
function render(dt) {
  ...
  if (needPanel) {
    ...
    rcv.setAttribute('aria-valuetext', S.kt <= 0
      ? 'no coverage floor; every event'
      : `carried by at least ${S.kt} Wikipedia language edition${S.kt === 1 ? '' : 's'}`);
    ccv.setAttribute('aria-valuenow', Math.round(S.win.t1));
    ccv.setAttribute('aria-valuetext', S.win.t1 >= T_MAX * 0.99
      ? 'all 75,000 years'
      : `${fmtYbpLabel(S.win.t1)} to ${fmtYbpLabel(S.win.t0)}`);
    needPanel = false;
  }
}
/* A render that throws is caught so the loop survives it - and until now that
   was the whole of it: a console.error and a canvas still showing its last good
   frame. From outside the page nothing changed. __BOOT_OK exists precisely so
   the smoke test can ask whether boot worked; there was no way to ask the same
   about the frame after it. Injecting a failure into drawEvents mid-session
   proved it: nothing escaped to window.onerror, __BOOT_OK stayed true, the
   canvas kept 1,997 distinct colours, and the only trace was a console line.
   That is the failure the sibling site lost a build to, arriving after boot
   rather than during it. Count it and keep the first one. */
window.__RENDER_ERRS = 0;
window.__RENDER_ERR = null;
function renderFailed(err) {
  window.__RENDER_ERRS++;
  if (!window.__RENDER_ERR) window.__RENDER_ERR = String((err && err.stack) || err);
...
function frame(now) {
  ...
  try { render(dt); } catch (err) { renderFailed(err); }
  requestAnimationFrame(frame);
}
function renderNow() {
  last = performance.now();
  try { render(0.016); } catch (err) { renderFailed(err); }
}
